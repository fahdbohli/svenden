...
if MODE == "live":
    ...
    # --- MODIFIED: Use the new GetSportsShortZip endpoint ---
    ODDS_URL = "https://tn.1xbet.com/service-api/LineFeed/GetSportsShortZip"
    ...
    # --- MODIFIED: Use parameters for the new endpoint ---
    ODDS_PARAMS = {
        "sports": f"{SPORT_ID}",  # sport id
        "champs": None,  # to be set per league
        "lng": "en",
...
def fetch_odds_for_match(session, match_id, parsing_rules):
    ...
    if MODE == "live":
        ...
        params = {
            ...
            "country": "187", "fcountry": "187", "countevents": "200"
        }
        ...
        params = {
            "id": match_id, "cfview": "0", "lng": "en", "GroupEvents": "true",
            "isSubGames": "false", "allEventsGroupSubGames": "false", "partner": "1",
            "country": "187", "fcountry": "187", "countevents": "300"
        }
...
def get_matches_for_league_prematch(session, champs_id, parsing_rules):
    ...
    # --- MODIFIED: Search both top-level and nested leagues for match data ---
    response_value = resp.json().get("Value")
    data = []  # Default to an empty list of matches

    if response_value and len(response_value) > 0:
        # This is the list of all league groups (e.g., "Club Friendlies", "England")
        league_groups = response_value[0].get("L", [])

        found = False
        for group in league_groups:
            # Case 1: The group itself is the target league (e.g. Club Friendlies)
            if group.get("LI") == int(champs_id):
                data = group.get("G", [])
                found = True
                break

            # Case 2: The group is a country containing sub-leagues (e.g. England)
            if "SC" in group:
                for sub_league in group.get("SC", []):
                    if sub_league.get("LI") == int(champs_id):
                        data = sub_league.get("G", [])
                        found = True
    ...
    for m in data:
        ...
        # The match ID is in the "I" key, not "CI"
        match_id = m.get("I")
        ts = m.get("S")
        if match_id is None or ts is None:
            continue
