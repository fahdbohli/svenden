...
MODE = "live"  # Choose the mode "live" or "prematch", use --mode
...
def main():
    ...
    results_by_country = {country_map.get(raw, raw): [] for raw in leagues_to_scrape.keys()}
    tasks = []
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for raw, leagues in leagues_to_scrape.items():
            clean_country = country_map.get(raw, raw)
            for league in leagues:
                # The check for SKIPPED_TOURNAMENTS is no longer needed here
    ...
    for clean_country, tournaments in results_by_country.items():
        if not tournaments:
            # This message will now only show for countries that were meant to be scraped but failed,
            # not for countries that were entirely skipped.
