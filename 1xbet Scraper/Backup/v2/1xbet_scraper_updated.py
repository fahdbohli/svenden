...
def create_session_with_retries():
    """
    Return a requests.Session configured to retry on
    transient errors (429, 500, 502, 503, 504, 529).
    """
    # ← Make sure this calls requests.Session(), not the function itself!
    session = requests.Session()
    session.headers.update(BASE_HEADERS)

    retry_strategy = Retry(
        total=10,                       # up to 5 retries
        backoff_factor=0.01,              # 0.3s, 0.6s, 0.9s, 1.2s…
        status_forcelist=[529],
...
def get_country_leagues(session):
    ...
    known_countries = [
        ...
        "Japan", "Tunisia", "China", "Israel", "Jamaica", "Jordan", "Rwanda", "Uganda", "Colombia", "Poland", "Estonia",
        "Abkhazia", "Australia", "South Sudan", "Mozambique", "Denmark", "Portugal", "Congo DR", "Argentina", "Algeria",
        "Brazil", "Chile", "Anguilla", "Aruba", "Faroe Islands", "Kuwait", "Serbia", "Switzerland", "Lithuania",
        "Uzbekistan",
        "Hong Kong", "Kenya", "Singapore", "Thailand", "Malaysia", "Ukraine", "Gambia", "Peru", "North Macedonia",
        "Morocco",
    ...
    for item in r.json().get("Value", []) or []:
        if item.get("SC"):  # Country with multiple leagues
            ...
            # Method 2: search for known countries in the league name
            for country in known_countries:
                if country in raw_name:
    ...
    for attempt in range(2):
        try:
            ...
            if e.response is not None and e.response.status_code == 529:
                wait = 5 * (attempt + 1)
                print(f"    529 encountered for {name}, sleeping {wait}s then retrying...")
                time.sleep(wait)
                continue
            raise
