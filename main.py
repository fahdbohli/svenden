...
# Load environment variables from .env if present so keys work without manual export
load_dotenv()

# Supabase configuration
SUPABASE_URL = os.getenv('SUPABASE_URL')
SUPABASE_KEY = os.getenv('SUPABASE_KEY')
EXPORT_TO_SUPABASE = bool(SUPABASE_URL and SUPABASE_KEY)

supabase_exporter = None
if EXPORT_TO_SUPABASE:
    supabase_exporter = SupabaseExporter(SUPABASE_URL, SUPABASE_KEY)
    print('[SUPABASE] Export is ENABLED')
else:
    print('[SUPABASE] Export is DISABLED (SUPABASE_URL/SUPABASE_KEY not set)')


# ----- Choose default configuration ---------------------------------------
MODE = "prematch"  # "prematch" or "live"
SPORT = "tennis"  # e.g., "football", "basketball", "tennis" ...
...
REVERSE_CHECKING = False  # default for reverse checking
# --------------------------------------------------------------------------
...
parser.add_argument(
    "--reverse-check", action="store_true", default=REVERSE_CHECKING,
    help="If set, enable reverse checking to detect and correct home/away team naming errors."
)
parser.add_argument(
    "--prune-remote", action="store_true", default=False,
    help="If set, delete remote files under the uploads prefix that are not present locally (use with care)."
)
...
REVERSE_CHECKING = args.reverse_check
# --------------------------------------------------------------------
...
# Get the directory containing main.py to resolve relative paths
MAIN_DIR = os.path.dirname(os.path.abspath(__file__))

# Resolve paths relative to main.py's directory
SOURCE_DIRECTORIES = [
    (entry["name"], os.path.normpath(os.path.join(MAIN_DIR, entry["path"].lstrip("./"))))
...
SWAP_WORDS_PATH = os.path.join("settings", SPORT, "swap_words.json")
if os.path.exists(SWAP_WORDS_PATH):
    matcher.load_swap_words(SWAP_WORDS_PATH)
    matcher.REVERSE_CHECKING = REVERSE_CHECKING
    if REVERSE_CHECKING:
        print("Reverse checking is ENABLED. Will detect and correct home/away team naming errors.")
else:
    if REVERSE_CHECKING:
        print(f"Warning: Reverse checking enabled but swap_words.json not found at {SWAP_WORDS_PATH}")
        REVERSE_CHECKING = False
        matcher.REVERSE_CHECKING = False
...
if CHECKING_MODE == "arb":
    ...
    with open(os.path.join("settings", SPORT, "full_check_markets.json"), encoding="utf-8") as fcfile:
        fc_root = json.load(fcfile)
        arb_calculator.FULL_CHECK_MARKETS = set(fc_root.get("full_check_markets", []))
    ...
    ev_calculator.ONLY_SHOW_EV_SOURCE_OPPS = ev_settings.get("ONLY_SHOW_EV_SOURCE_OPPS", False)
...
def process_files_optimal():
        ...
        for group in matching_groups:
            ...
            if group_object:
                ...
                for opp in opps_list:
                    # If the only show ev source opps setting is enabled, only proceed if the overprice source has been
                    # identified and it matches the designated EV_SOURCE.
                    if (CHECKING_MODE == "ev" and
                        ev_calculator.ONLY_SHOW_EV_SOURCE_OPPS and
                        opp.get("overprice_source") != ev_calculator.EV_SOURCE):
                        continue
    ...
    # Write results to files and export to Supabase if configured
    for country, list_of_groups in sorted(results_by_country.items()):
        if list_of_groups:
            ...
            # Export vers Supabase si configuré
            if EXPORT_TO_SUPABASE and supabase_exporter:
                try:
                    # Aplatir la structure pour l'export
                    opportunities = []
                    for group in list_of_groups:
                        for opp in group.get('opportunities', []):
                            # Enrichir l'opportunité avec des données supplémentaires
                            enriched_opp = {
                                'date': group.get('date'),
                                'time': group.get('time'),
                                'home_team': group.get('home_team'),
                                'away_team': group.get('away_team'),
                                'profit_margin': opp.get('margin'),
                                'bookmakers': {
                                    'odds': opp.get('odds', {}),
                    ...
                    if opportunities:
                        # Debug: show what we're sending to Supabase
                        try:
                            print(f"\n[SUPABASE DEBUG] Preparing to send {len(opportunities)} opportunities for {country}")
                            print(f"[SUPABASE DEBUG] Sample: {opportunities[0]}")
                        except Exception:
                            pass
                        results = supabase_exporter.export_opportunities(opportunities)
                        print(f"\n[SUPABASE] Export pour {country}: {results['success']} succès, {results['failed']} échecs")
                except Exception as e:
                    print(f"\n[SUPABASE] Erreur lors de l'export pour {country}: {str(e)}")
    ...
    # After writing and exporting to DB, upload generated JSON files to Supabase Storage
    if EXPORT_TO_SUPABASE and supabase_exporter:
        bucket_name = 'uploads'
        print(f"\nUploading generated JSON files from '{OUTPUT_DIR}' to Supabase bucket '{bucket_name}'...")

        # Walk OUTPUT_DIR recursively and upload each JSON file, preserving the
        # relative path under the desired prefix so remote path becomes:
        # arb_output/arb_opportunities/{MODE}/{SPORT}/{relpath}
        # Build a set of expected remote paths to allow pruning stale remote files
        expected_remote = set()
        pruned_after_sync = False
        for root, _, files in os.walk(OUTPUT_DIR):
            for fname in sorted(files):
                if not fname.endswith('.json'):
                    continue
                # skip internal cache and hidden files and activity tracker
                if fname.startswith('_') or fname == os.path.basename(ACTIVITY_TRACKER_PATH):
                    continue
                ...
                try:
                    ...
                    # relpath relative to OUTPUT_DIR
                    rel = os.path.relpath(local_path, OUTPUT_DIR).replace(os.sep, '/')
                    remote_path = f"arb_output/arb_opportunities/{MODE}/{SPORT}/{rel}"

                    # Track this path as expected on remote so we can prune leftovers later
                    expected_remote.add(remote_path)

                    ok, resp = supabase_exporter.upload_file_to_storage(bucket_name, remote_path, content)
                    if ok:
                        print(f"[STORAGE] Uploaded {local_path} -> {bucket_name}/{remote_path}")
                        continue

                    # If upload failed, attempt to create the bucket and retry once
                    print(f"[STORAGE] Upload failed for {local_path}: {resp}. Trying to create bucket '{bucket_name}'...")
                    create_res = supabase_exporter.create_bucket_if_missing(bucket_name)
                    if isinstance(create_res, dict) and create_res.get('error'):
                        print(f"[STORAGE] Could not create bucket: {create_res.get('error')}")
                        continue

                    ok2, resp2 = supabase_exporter.upload_file_to_storage(bucket_name, remote_path, content)
                    if ok2:
                        print(f"[STORAGE] Uploaded {local_path} after creating bucket -> {bucket_name}/{remote_path}")
                    else:
                        print(f"[STORAGE] Upload still failed for {local_path}: {resp2}")

                except Exception as e:
                    print(f"[STORAGE] Error reading or uploading {local_path}: {str(e)}")

        # Prune remote files that are not present locally. This will run after every cycle
        # so that stale files are removed automatically.
        try:
            prefix = f"arb_output/arb_opportunities/{MODE}/{SPORT}/"
            ok, res = supabase_exporter.prune_remote_files(bucket_name, expected_remote, prefix)
            if ok:
                print(f"\n[PRUNE] Pruned remote files: deleted={len(res.get('deleted', []))}, failed={len(res.get('failed', []))}")
            else:
                print(f"\n[PRUNE] Prune failed: {res}")
            pruned_after_sync = True
        except Exception as e:
            print(f"\n[PRUNE] Exception during prune: {e}")

        # Optionally prune remote files using legacy flag if prune wasn't done above
        if PRUNE_REMOTE and not pruned_after_sync:
            prefix = f"arb_output/arb_opportunities/{MODE}/{SPORT}/"
            print(f"\n[PRUNE] Listing remote objects under prefix '{prefix}' to prune outdated files...")
            remote_list = supabase_exporter.list_files(bucket_name, prefix)
            if isinstance(remote_list, dict) and remote_list.get('error'):
                print(f"[PRUNE] Could not list remote objects: {remote_list.get('error')}")
            else:
                # Build a set of expected remote paths from local files
                expected = set()
                for root, _, files in os.walk(OUTPUT_DIR):
                    for fname in files:
                        if not fname.endswith('.json'):
                        ...
                        local_path = os.path.join(root, fname)
                        rel = os.path.relpath(local_path, OUTPUT_DIR).replace(os.sep, '/')
                        expected.add(f"{prefix}{rel}")

                # Remove remote files not in expected
                for remotepath in remote_list:
                    if remotepath not in expected:
                        print(f"[PRUNE] Removing remote file not present locally: {remotepath}")
                        ok, resp = supabase_exporter.remove_file(bucket_name, remotepath)
                        if not ok:
                            print(f"[PRUNE] Failed to remove {remotepath}: {resp}")
