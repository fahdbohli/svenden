            ...
            for src2 in sources:
                ...
                for m2 in matches_by_source[src2]:
                    ...
                    if not (date_match and time_match):
                        continue
                    # ===================== CORE NAME GATEKEEPER & SCORING =====================
                    # Get the essential part of the team names, without qualifiers like "U20"
                    ...
                    # If both core names are empty after stripping, they are not valid for comparison
                    if (not core_h1 and not core_h2) or (not core_a1 and not core_a2):
                        continue

                    # Calculate similarity scores based on the CORE names.
                    # This is the key fix: using get_core_name instead of simplify_team_name.
                    ...
                    # STRENGTHENED GATEKEEPER:
                    # Reject matches where one of the pairs is a very poor fit.
                    # This prevents a perfect away-team match from carrying a terrible home-team match.
                    # We use a lower threshold here (e.g., 0.45) just to discard obvious nonsense.
                    if min(home_score, away_score) < 0.45:
                        continue
                    # ===========================================================================


                    # This block is now comparing the much more accurate core scores.
                    passed = False
                    # Ensure your thresholds are single float values, not sets.
                    strong_thresh = list(STRONG_THRESHOLD)[0] if STRONG_THRESHOLD else 0.85
                    moderate_thresh = list(MODERATE_THRESHOLD)[0] if MODERATE_THRESHOLD else 0.60

                    if ((home_score >= strong_thresh and away_score >= moderate_thresh) or
                            (away_score >= strong_thresh and home_score >= moderate_thresh)):
                        passed = True
