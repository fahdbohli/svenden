...
# ---------------------------------------------------
# LIVE
# Note: The Live scraping logic is preserved and untouched as requested.
# If you need similar group-based fetching for Live mode, it would require
# parallel modifications to scrape_live and its helpers.
# ---------------------------------------------------
...
def parse_event_details(event_json, parsing_config):
    ...
    for m in event_json.get("markets", []):
        ...
        for market_key, market_data in all_markets_config.items():
            if m_name in market_data.get("market_names", []):
...
def scrape_live(loop, inversed, delay, sport="football"):
    ...
    while True:
        ...
        # Modified to work with new config structure
