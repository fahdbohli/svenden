...
SPORT = "tennis"              # Default sport to scrape. Use --sport to change it.
...
SCRAPE_BY_ID = True            # ADD THIS: If True (and SCRAPE_PER_COUNTRY is True), scrapes match-by-match. Use --scrape-by-id to enable.
...
BONUS_RATIO = "1.15"            # bonus ratio value (x1.1, x1.15, ...)
MINIMUM_BONUS_ODD = "1.5"       # the minimum odd value for the bonus to apply
MERGE_FILES = True             # Set to True to merge all country files into one. Use --merge-files to override.
