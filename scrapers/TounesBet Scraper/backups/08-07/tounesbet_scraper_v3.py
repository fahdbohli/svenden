...
BONUS = True                    # Default state for the 1.1x bonus. Use --bonus to enable it.
TOURNAMENTS_PER_REQUEST = 10    # How many tournaments to fetch in a single request
NUM_SESSIONS = 15               # Sessions with unique ddos codes number
MAX_WORKERS = 12                 # Workers number
LOOP = False                    # Loop state, use --loop to enable it
DELAY = 5                       # Delay in seconds between each cycle if loop is activated
# ---------------------------------------------------------------------------------
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=SPORT,
        help='The sport to scrape. Currently only "football" is supported.'
    )
    # Use BooleanOptionalAction to get --bonus and --no-bonus flags.
    # The default is taken from the global config. Requires Python 3.9+
    parser.add_argument(
        '--bonus',
        action=argparse.BooleanOptionalAction,
        default=BONUS,
        help='Enable the 1.1x odds bonus for odds > 1.5. Disabled by default.'
    )
    parser.add_argument(
        '--loop',
        action='store_true',
    ...
    SPORT_IDS = {'football': 1181}
    sport_name = args.sport.lower()
    if sport_name not in SPORT_IDS:
        safe_print(f"Error: Sport '{sport_name}' is not supported. Supported sports: {list(SPORT_IDS.keys())}")
        return
    ...
    while True:
        ...
        if not countries:
            safe_print("No countries found. Exiting this cycle.")
            if not args.loop:
                return
            else:
                safe_print(f"Will retry after {args.delay} seconds...")
        ...
        with ThreadPoolExecutor(max_workers=actual_workers) as executor:
            process_args = [(country, session_manager, sport_id, base_output_dir, tournaments_per_request) for country in countries]
