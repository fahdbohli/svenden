...
DEBUG_SOURCE_NAME = "Asbet"  # The 'source' of the match to debug
DEBUG_MATCH_ID = "27526405"  # The 'match_id' of the match to debug
# ---------------------
    ...
    for src1 in sources:
        for m1 in matches_by_source[src1]:
            ...
            if is_debug_target:
                print("\n" + "=" * 80)
                print(
                    f"[DEBUG] Initiating search for Target: {m1['source']} ID:{mid1} ({m1['home_team']} vs {m1['away_team']})")
                print("=" * 80)
            ...
            for src2 in sources:
                ...
                for m2 in matches_by_source[src2]:
                    ...
                    _log_debug(is_debug_target,
                               f"Comparing with: {m2['source']} ID:{mid2} ({m2['home_team']} vs {m2['away_team']})")
                    ...
                    if not d1 or not d2 or abs((d1 - d2).days) > DAY_DIFF_TOLERANCE:
                        _log_debug(is_debug_target,
                                   f"  -> FAILED [Date Guard]: Mismatch. D1='{d1}', D2='{d2}', Tol='{DAY_DIFF_TOLERANCE} days'")
                        continue
                    ...
                    try:
                        ...
                        if abs((dt1 - dt2).total_seconds()) / 60 > TIME_DIFF_TOLERANCE:
                            _log_debug(is_debug_target,
                                       f"  -> FAILED [Time Guard]: Mismatch. T1='{t1}', T2='{t2}', Tol='{TIME_DIFF_TOLERANCE} mins'")
                            continue
                    except ValueError:
                        if t1 != t2:
                            _log_debug(is_debug_target,
                                       f"  -> FAILED [Time Guard]: Mismatch. T1='{t1}' != T2='{t2}' (unparsable)")
                            continue

                    _log_debug(is_debug_target, "  -> PASSED [Date/Time Guards]")

                    # Important-term guard (CORRECTED)
                    ...
                    for grp in IMPORTANT_TERM_GROUPS:
                        present_in_1 = any(re.search(r'\b' + re.escape(term.lower()) + r'\b', text1) for term in grp)
                        present_in_2 = any(re.search(r'\b' + re.escape(term.lower()) + r'\b', text2) for term in grp)
                        if present_in_1 != present_in_2:
                            group_mismatch = True
                            break

                    if group_mismatch:
                        _log_debug(is_debug_target, f"  -> FAILED [Important Terms Guard]: Mismatch detected.")
                        continue
                    _log_debug(is_debug_target, "  -> PASSED [Important Terms Guard]")
                    ...
                    if is_debug_target:
                        _debug_match_details(m1, m2, home_score, away_score)

                    if min(home_score, away_score) < GATEKEEPER_THRESHOLD:
                        _log_debug(is_debug_target,
                                   f"  -> FAILED [Gatekeeper Guard]: Min score below {GATEKEEPER_THRESHOLD}")
                        continue
                    _log_debug(is_debug_target, f"  -> PASSED [Gatekeeper Guard]")
                    ...
                    if not passed:
                        _log_debug(is_debug_target,
                                   f"  -> FAILED [Strong/Moderate Thresholds]: Strong={STRONG_THRESHOLD}, Mod={MODERATE_THRESHOLD}")
                        continue
                    _log_debug(is_debug_target, f"  -> PASSED [Strong/Moderate Thresholds]")

                    avg_score = (home_score + away_score) / 2
                    if avg_score > best_score:
                        _log_debug(is_debug_target,
                                   f"  -> SUCCESS: New best candidate found. Avg Score: {avg_score:.3f} > old best {best_score:.3f}")
                        best_score = avg_score
                        best_match = m2
                    else:
                        _log_debug(is_debug_target,
                                   f"  -> INFO: Candidate passed checks but score ({avg_score:.3f}) not higher than current best ({best_score:.3f}). Skipping.")

                if best_match:
                    if is_debug_target:
                        print(
                            f"\n*** Found best match in source '{src2}': ID {best_match.get('match_id')} with score {best_score:.3f} ***\n")
                    group.append(best_match)
                    processed[best_match['source']].add(str(best_match.get('match_id')))
                else:
                    if is_debug_target:
                        print(f"--- No suitable match found in source '{src2}' for this iteration ---")
