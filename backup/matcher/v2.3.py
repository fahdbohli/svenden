        ...
        for m1 in matches_by_source[src1]:
            ...
            for src2 in sources:
                ...
                for m2 in matches_by_source[src2]:
                    ...
                    core_home_ratio = difflib.SequenceMatcher(None, core_h1, core_h2).ratio()
                    core_away_ratio = difflib.SequenceMatcher(None, core_a1, core_a2).ratio()

                    # GATEKEEPER: At least one of the team pairs must have a decent core similarity.
                    # This prevents matching "Sobradinho" with "America Mineiro" just because
                    # they both have "U20". A threshold of 0.55 is a good starting point.
                    if core_home_ratio < 0.52 and core_away_ratio < 0.52:
                        continue
                    # ====================================================================

                    home_score = difflib.SequenceMatcher(
                        None, simplify_team_name(h1), simplify_team_name(h2)
                    ).ratio()
                    away_score = difflib.SequenceMatcher(
                        None, simplify_team_name(a1), simplify_team_name(a2)
                    ).ratio()
