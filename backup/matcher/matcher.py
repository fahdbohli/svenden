...
def calculate_jaccard_score(name1: str, name2: str) -> float:
    """
    Calculates the Jaccard similarity between the sets of words of
    two core team names. This provides a more robust score than simple
    string comparison, as it's less sensitive to word order and shared
    unimportant words.
    """
    ...
    # If both sets are empty, they are a perfect match
    ...
    # Avoid division by zero if the union is empty (should only happen if both inputs are empty)
    if not union:
        return 0.0
...
def find_all_matching_matches(
        ...
        for m1 in matches_by_source[src1]:
            ...
            for src2 in sources:
                ...
                for m2 in matches_by_source[src2]:
                    ...
                    # ===================== NEW JACCARD-BASED SCORING LOGIC =====================
                    # Use synonym check to force a perfect score of 1.0
                    home_is_synonym = check_team_synonyms(h1, h2)
                    away_is_synonym = check_team_synonyms(a1, a2)

                    # Use the new Jaccard score for a more robust, word-set based comparison.
                    home_score = 1.0 if home_is_synonym else calculate_jaccard_score(h1, h2)
                    away_score = 1.0 if away_is_synonym else calculate_jaccard_score(a1, a2)
                    # =========================================================================
    ...
    all_groups = [list(g.values()) for g in match_index.values() if len(g) >= 2] + fuzzy_matches

    # Helper to compute a unique key for each match dict
    def match_key(m):
    ...
    # Merge any groups that share at least one match key
    merged = []
    for group in all_groups:
        # the set of keys in this new group
        group_keys = { match_key(m) for m in group }
        # see if it overlaps an existing merged group
        merged_into = False
        for mgroup in merged:
            mgroup_keys = { match_key(m) for m in mgroup }
            if group_keys & mgroup_keys:
                # append only those matches not already in mgroup
                existing_keys = mgroup_keys
                for m in group:
                    if match_key(m) not in existing_keys:
                        mgroup.append(m)
                merged_into = True
                break
        if not merged_into:
