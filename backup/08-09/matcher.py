        ...
        for m1 in matches_by_source[src1]:
            ...
            for src2 in sources:
                ...
                for m2 in matches_by_source[src2]:
                    ...
                    # NEW, STRICTER Important-term guard
                    if not _check_important_terms_match(m1['home_team'], m2['home_team']):
                        continue
                    if not _check_important_terms_match(m1['away_team'], m2['away_team']):
                        continue
                ...
                for m1b in matches_by_source[src1]:
                    ...
                    # ----- START OF CORRECTED BLOCK (REVERSE CHECK) -----
                    # NEW, STRICTER Important-term guard (for reverse check)
                    if not _check_important_terms_match(m1b['home_team'], best_match['home_team']):
                        continue
                    if not _check_important_terms_match(m1b['away_team'], best_match['away_team']):
                        continue
                    # ----- END OF CORRECTED BLOCK (REVERSE CHECK) -----

                    # This part had a logic error where it was checking the old combined text.
                    # The new check above replaces the need for the complex loop here.
                    home_score_rev = (
