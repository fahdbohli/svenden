...
TOURNAMENTS_PER_REQUEST = 15    # How many tournaments to fetch in a single request
...
def process_country(country_info):
    ...
    country, session_manager, sport_id, base_output_dir, tournaments_per_request, parsing_config = country_info
    ...
    try:
        ...
        for i, chunk in enumerate(tournament_id_chunks):
            ...
            # --- 2. Fetch Special Market Odds (Totals, Handicaps, etc.) ---
            for market_config in parsing_config.get_special_markets():
                market_desc = market_config.get('description', 'special market')
                odd_type_id = market_config.get('odd_type_id')
                    ...
                    if special_lines_data:
                        for match_id, odds in special_lines_data.items():
                            if match_id in all_matches_data:
                                # Remove default U/O 2.5 if it's going to be replaced by more specific lines from the 'totals' market
        ...
        # Re-structure data into tournament groups
        grouped_matches = {}
        ...
        # Add timestamp at the beginning of the data
        data_with_timestamp = [
            {"last_updated": current_time}
        ] + country_data
...
def main():
    ...
    while True:
        base_output_dir = f"scraped_prematch_matches/{SPORT}"
        ...
        with ThreadPoolExecutor(max_workers=actual_workers) as executor:
            # --- UPDATED: Pass parsing_config to the worker ---
            process_args = [
                (country, session_manager, sport_id, base_output_dir, tournaments_per_request, parsing_config)
