...
def extract_matches(html, parsing_config):
    ...
    Returns a tuple: (list_of_matches, dict_of_tournament_names).
    """
    soup = BeautifulSoup(html, "html.parser")
    matches = []
    scraped_tournament_names = {}  # New: To store tournament names found on the page
    ...
    for tr in tbody.find_all("tr", recursive=False):
        ...
        if "header_tournament_row" in classes:
            # New: Logic to extract tournament names
            ...
            if t_id and name_td:
                ...
                if name_div:
                    # Parse "Country / Tournament Name" format
            ...
            # Basic match info (this part is unchanged)
            ...
            # Data-driven odds extraction (this part is unchanged)
    ...
    try:
        ...
        if scrape_per_country:
            ...
            try:
                ...
                # Note the new tuple unpacking here
                matches_from_main, scraped_tournament_names = extract_matches(response_main.text, parsing_config)
        ...
        country_data = []
        if scrape_per_country:
            # For per-country mode, create a definitive name map.
            # Start with the initial API list...
            tournament_name_map = {t['tournament_id']: t['tournament_name'] for t in initial_tournaments}
            # ...then overwrite with the more accurate names scraped from the page.
            tournament_name_map.update(scraped_tournament_names)

            for t_id, matches in grouped_matches.items():
                # Now, get the best possible name, falling back to a placeholder if somehow still not found.
            ...
            # For the original mode, the logic remains unchanged.
